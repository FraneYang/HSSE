    ...
    public partial class TrainType : PageBase
    {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (this.judgementDelete(rowID, true))
            {
                ...
                BLL.LogService.AddLog(this.CurrUser.UserId, "删除培训类型");
                // 重新绑定表格，并模拟点击[新增按钮]
                BindGrid();
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            Model.Base_TrainType trainType = new Model.Base_TrainType
            {
            ...
            {
                trainType.TrainTypeId = SQLHelper.GetNewID(typeof(Model.Base_TrainType));
                BLL.TrainTypeService.AddTrainType(trainType);
                BLL.LogService.AddLog(this.CurrUser.UserId, "添加培训类型");
            }
            else
            {
                trainType.TrainTypeId = strRowID;
                BLL.TrainTypeService.UpdateTrainType(trainType);
                BLL.LogService.AddLog(this.CurrUser.UserId, "修改培训类型");
            }
            this.SimpleForm1.Reset();
            // 重新绑定表格，并点击当前编辑或者新增的行
            BindGrid();
            PageContext.RegisterStartupScript(String.Format("F('{0}').selectRow('{1}');", Grid1.ClientID, trainType.TrainTypeId));
        }
        ...
        {
            var buttonList = BLL.CommonService.GetAllButtonList(this.CurrUser.UserId, BLL.Const.TrainTypeMenuId);
        ...
        private bool judgementDelete(string id, bool isShow)
        {
            ...
            if (Funs.DB.Training_TrainRecord.FirstOrDefault(x => x.TrainTypeId == id) != null)
            {
                content = "该培训类别已在【人员培训】中使用，不能删除！";
            }
