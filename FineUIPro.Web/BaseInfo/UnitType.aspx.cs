...
namespace FineUIPro.Web.BaseInfo
{
    public partial class UnitType : PageBase
    {
        ...
        {
            var buttonList = BLL.CommonService.GetAllButtonList( this.CurrUser.UserId, BLL.Const.UnitTypeMenuId);
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            if (judgementDelete(hfFormID.Text, true))
            {
                ...
                BLL.LogService.AddLog(this.CurrUser.UserId, "删除单位类别");
                // 重新绑定表格，并模拟点击[新增按钮]
                BindGrid();
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(strRowID))
            {
                ...
                BLL.LogService.AddLog(this.CurrUser.UserId, "添加单位类别");
            }
            else
            {
                depart.UnitTypeId = strRowID;
                BLL.UnitTypeService.UpdateUnitType(depart);
                BLL.LogService.AddLog(this.CurrUser.UserId, "修改单位类别");
            }
