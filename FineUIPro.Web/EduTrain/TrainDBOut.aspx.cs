namespace FineUIPro.Web.EduTrain
{
    ...
    public partial class TrainDBOut : PageBase
    {
        /// <summary>
        /// 加载页面
        /// </summary>
        ...
        {
            string strSql = @"SELECT TrainingItemId,Training.TrainingCode,Training.TrainingName,TrainingItemCode,TrainingItemName,Abstracts,AttachUrl,VersionNum,TestType,InstallationIds "
                        + @" ,(CASE WHEN TestType = '1' THEN '单选题' WHEN TestType = '2' THEN '多选题' ELSE '判断题' END) AS TestTypeName,InstallationNames,AItem,BItem,CItem,DItem,EItem "
                        + @" ,Score,Replace(Replace(Replace(Replace(Replace(AnswerItems,'1','A'),'2', 'B'),'3', 'C'),'4', 'D'),'5', 'E') AS AnswerItems "
                        + @" FROM dbo.Training_TrainingItem AS Item"
                        + @" LEFT JOIN Training_Training AS Training ON Item.TrainingId=Training.TrainingId"
                        + @" WHERE 1 = 1";
