namespace FineUIPro.Web.EduTrain
{
    ...
    public partial class TrainingItemIn : PageBase
    {
        #region 定义变量
        /// <summary>
        /// 上传预设的虚拟路径
        ...
        /// 导入集合
        /// </summary>
        public static List<Model.View_Training_TrainingItem> viewTrainingItems = new List<Model.View_Training_TrainingItem>();
        ...
        private bool AddDatasetToSQL(DataTable pds)
        {
            ...
            {
                ///试题类型
                var trainings = from x in Funs.DB.Training_Training select x;
                ///岗位（装置/科室）
                var Installations = from x in Funs.DB.Base_Installation select x;             
                for (int i = 0; i < ir; i++)
                {
                    Model.View_Training_TrainingItem newViewTrainingItem = new Model.View_Training_TrainingItem
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            newViewTrainingItem.TrainingId = newTraining.TrainingId = SQLHelper.GetNewID(typeof(Model.Training_Training));
                            newViewTrainingItem.TrainingName = newTraining.TrainingName = col1;
                            newTraining.TrainingCode = newViewTrainingItem.TrainingCode;
                            BLL.TrainingService.AddTraining(newTraining);
                        }
                    }
                    ///试题题型
                    ...
                    if (!string.IsNullOrEmpty(col4))
                    {
                        ...
                        if (InstallationSels.Count() > 0)
                        {
                            ...
                            {
                                newViewTrainingItem.InstallationNames = col4;
                                newViewTrainingItem.InstallationIds = ids.Substring(0, ids.LastIndexOf(","));
                            }
                        }
                    }
                    ...
                    {
                        var addTrainingItem = Funs.DB.Training_TrainingItem.FirstOrDefault(x => x.Abstracts == newViewTrainingItem.Abstracts && x.TrainingId == newViewTrainingItem.TrainingId && x.InstallationIds == newViewTrainingItem.InstallationIds);
                        if (addTrainingItem == null)
                        {
                            Model.Training_TrainingItem newTrainingItem = new Model.Training_TrainingItem
                            {
                                TrainingItemId = newViewTrainingItem.TrainingItemId = SQLHelper.GetNewID(typeof(Model.Standard_HSSEStandard)),
                                ...
                                InstallationIds = newViewTrainingItem.InstallationIds,
                                InstallationNames = newViewTrainingItem.InstallationNames,
                            ...
                            BLL.TrainingItemService.AddTrainingItem(newTrainingItem);
                            ///加入培训试题库
        ...
        protected void Window1_Close(object sender, WindowCloseEventArgs e)
        {
            ...
            {
                viewTrainingItems = Session["trainingItem"] as List<Model.View_Training_TrainingItem>;
            }
            if (viewTrainingItems.Count > 0)
            {
                this.Grid1.Hidden = false;
        ...
        protected void PageManager1_CustomEvent(object sender, CustomEventArgs e)
        {
            ...
            {
                string rootPath = Server.MapPath("~/");
                string filePath = Const.TrainingTemplateUrl;
