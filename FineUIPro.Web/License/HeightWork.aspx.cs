namespace FineUIPro.Web.License
{
    ...
    public partial class HeightWork : PageBase
    {
        /// <summary>
        /// 加载页面
        /// </summary>
        ...
        private void BindGrid()
        {
            string strSql = @"SELECT HeightWorkId,LicenseCode,(CASE WHEN ApplyInstallation.InstallationName IS NULL THEN ApplyUnit.UnitName ELSE ApplyInstallation.InstallationName END) AS ApplyUintName,JobContent"
                            ...
                            + @" LEFT JOIN Base_Unit AS ApplyUnit ON HeightWork.UnitId =ApplyUnit.UnitId "
                            + @" LEFT JOIN Base_Installation AS ApplyInstallation ON HeightWork.InstallationId =ApplyInstallation.InstallationId "
                            + @" LEFT JOIN Sys_User AS Users ON HeightWork.ApplyManId =Users.UserId"
                            + @" LEFT JOIN Base_Installation AS HeightWorkInstallation ON HeightWork.JobInstallationId =HeightWorkInstallation.InstallationId "                            
                            + @" WHERE 1=1 ";
        ...
        private void DeleteData()
        {
            if (Grid1.SelectedRowIndexArray.Length > 0)
            {
                foreach (int rowIndex in Grid1.SelectedRowIndexArray)
                {
                    ...
                    BLL.HeightWorkService.DeleteHeightWorkById(rowID);
                    BLL.LogService.AddLog(this.CurrUser.UserId, "删除高处安全作业证");
                    //var HeightWorks = BLL.HeightWorkService.GetHeightWorkById(rowID);
                    //if (HeightWorks != null)
                    //{                        
                    //}
                }
