    ...
    public partial class PersonalInfo : PageBase
    {
        ...
        private void Tab1LoadData()
        {
            ...
            if (user != null)
            {
                ...
                var sexVules = BLL.ConstValue.drpConstItemList(ConstValue.Group_Gender).FirstOrDefault(x => x.ConstValue == user.Sex);
                if (sexVules != null)
                {
                ...
                var MarriageVules = BLL.ConstValue.drpConstItemList(ConstValue.Group_Marriage).FirstOrDefault(x => x.ConstValue == user.Marriage);
                if (MarriageVules != null)
                {
                    this.drpMarriage.Text = MarriageVules.ConstText;
                }

                var NationVules = BLL.ConstValue.drpConstItemList(ConstValue.Group_Nation).FirstOrDefault(x => x.ConstValue == user.Nation);
                if (NationVules != null)
                {
                ...
                var EducationVules = BLL.ConstValue.drpConstItemList(ConstValue.Group_Education).FirstOrDefault(x => x.ConstValue == user.Education);
