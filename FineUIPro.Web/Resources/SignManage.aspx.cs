...
namespace FineUIPro.Web.Resources
{
    public partial class SignManage : PageBase
    {
        ...
        {
            string strSql = @"SELECT SignManage.SignManageId,
                                     SignManage.SignName,
                                     SignManage.SignCode,
                                     SignManage.SignLen,
                                     SignManage.SignWide,
                                     SignManage.SignHigh,
                                     SignManage.SignThick,
                                     SignManage.Material,
                                     SignManage.SignArea,
                                     SignManage.SignType,
                                     Const.ConstText AS SignTypeName,
                                     SignManage.SignImage,
                                     SignManage.SignUrl"
                           + @" FROM Resource_SignManage AS SignManage"
        ...
        private void DeleteData()
        {
            ...
            if (Grid1.SelectedRowIndexArray.Length > 0)
            {
                foreach (int rowIndex in Grid1.SelectedRowIndexArray)
                {
                    ...
                    BLL.LogService.AddLog(this.CurrUser.UserId, "删除标牌管理");
                }
                BindGrid();
                ShowNotify("删除数据成功!");
            }
        ...
        {
            var buttonList = BLL.CommonService.GetAllButtonList(this.CurrUser.UserId, BLL.Const.SignManageMenuId);
