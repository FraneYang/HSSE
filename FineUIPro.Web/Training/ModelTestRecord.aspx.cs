...
namespace FineUIPro.Web.Training
{
    public partial class ModelTestRecord : PageBase
    {
        ...
        {
            string strSql = @"SELECT ModelTestRecord.ModelTestRecordId,ModelTestRecord.TrainingId, ModelTestRecord.TestManId, ModelTestRecord.TestStartTime, ModelTestRecord.TestEndTime, ModelTestRecord.TestScores,Training.TrainingName,Users.UserName AS TestManName"
                         + @" ,'100' as TotalScore,'90' as Duration,'95' as QuestionCount"
                         + @" FROM dbo.Training_ModelTestRecord AS ModelTestRecord"
                         + @" LEFT JOIN dbo.Training_Training AS Training ON Training.TrainingId=ModelTestRecord.TrainingId"
                         + @" LEFT JOIN dbo.Sys_User AS Users ON Users.UserId = ModelTestRecord.TestManId WHERE 1=1";
            ...
            {
                strSql += " AND (Users.UserName LIKE @TestManName OR Training.TrainingName LIKE @TestManName)";
        ...
        private void DeleteData()
        {
            if (Grid1.SelectedRowIndexArray.Length > 0)
            {
                ...
                BLL.LogService.AddLog(this.CurrUser.UserId, "删除模拟考试");
                BindGrid();
                ShowNotify("删除数据成功!");
            }
        }
