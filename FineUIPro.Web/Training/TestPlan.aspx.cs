...
namespace FineUIPro.Web.Training
{
    public partial class TestPlan :PageBase
    {
        ...
        {
            string strSql = @"SELECT TestPlan.TestPlanId,TestPlan.PlanName,TestPlan.PlanCode,TestPlan.PlanManId,PlanMan.UserName AS PlanManName,TestPlan.PlanDate,TestPlan.TestStartTime,TestPlan.TestEndTime,TestPlan.Duration,TestPlan.TotalScore,TestPlan.QuestionCount,TestPlan.TestPalce,TestPlan.UnitIds,TestPlan.UnitNames,TestPlan.DepartIds,TestPlan.DepartNames,TestPlan.InstallationIds,TestPlan.InstallationNames,TestPlan.WorkPostIds,TestPlan.WorkPostNames,TestPlan.QRCodeUrl,TestPlan.States"
            ...
            {
                strSql += " AND (TestPlan.PlanName LIKE @name OR TestPlan.PlanCode LIKE @name OR PlanMan.UserName LIKE @name OR TestPlan.TestPalce LIKE @name OR TestPlan.InstallationNames LIKE @name)";
        ...
        private void DeleteData()
        {
            if (Grid1.SelectedRowIndexArray.Length > 0)
            {
                ...
                BLL.LogService.AddLog(this.CurrUser.UserId, "删除考试计划");
                BindGrid();
        ...
        {
            var buttonList = BLL.CommonService.GetAllButtonList(this.CurrUser.UserId, BLL.Const.TestPlanMenuId);
        ...
        private void ShowQurl(string strValue, string title)
        {
            ...
            PageContext.RegisterStartupScript(Window1.GetShowReference(String.Format("../Controls/ShowQRImage.aspx?strValue={0}&urlName={1}&title={2}", strValue, urlName, System.Web.HttpUtility.UrlEncode(title), "查看 - "), "二维码", 300, 300));
        }
    }
}