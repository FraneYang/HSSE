    ...
    public partial class TestRecord : PageBase
    {
        ...
        private void BindGrid()
        {
            string strSql = @"SELECT TestRecord.TestRecordId,TestRecord.TestPlanId, TestRecord.TestManId,TestRecord.TestStartTime,TestRecord.TestEndTime, TestRecord.TestScores,
                            ...
                            ISNULL(TestPlan.Duration,90) AS Duration,ISNULL(TestPlan.TotalScore,100) AS TotalScore,TestPlan.TestPalce,ISNULL(TestPlan.QuestionCount,95) AS QuestionCount,TestRecord.TemporaryUser,Users.UserName AS TestManName"
                         ...
                         + @" LEFT JOIN dbo.Training_Training AS Training ON Training.TrainingId = TestRecord.TestType"
                         + @" LEFT JOIN dbo.Sys_User AS Users ON Users.UserId = TestRecord.TestManId WHERE (isFiled IS NULL OR isFiled = 0)";
            List<SqlParameter> listStr = new List<SqlParameter>();          
            if (!string.IsNullOrEmpty(this.txtName.Text.Trim()))
            {
                strSql += " AND (Users.UserName LIKE @name OR TestPlan.PlanName LIKE @name OR Training.TrainingName LIKE @name)";
           ...
           // 
            ...
            for (int i = 0; i < Grid1.Rows.Count; i++)
            {
                ...
                if (testRecord != null)
                {
                    if (!testRecord.TestScores.HasValue || testRecord.TestScores < 80 )
                    {
        ...
        private void DeleteData()
        {
            if (Grid1.SelectedRowIndexArray.Length > 0)
            {
                ...
                BLL.LogService.AddLog(this.CurrUser.UserId, "删除考试记录");
                BindGrid();
                ShowNotify("删除数据成功!");
            }
        }
        ...
        {
            var buttonList = BLL.CommonService.GetAllButtonList(this.CurrUser.UserId, BLL.Const.TestRecordMenuId);
