...
namespace FineUIPro.Web.Training
{
    public partial class TestRecordItem : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (this.CurrUser.UserId == BLL.Const.sysglyId)
                {
                    ...
                    if (testRecord != null && !testRecord.TestEndTime.HasValue)
                    {
        ...
        protected void btnClose_Click(object sender, EventArgs e)
        {
            ...
            if (testRecord != null)
            {
                var sumSubjectScore = Funs.DB.Training_TestRecordItem.Where(x => x.TestRecordId == this.TestRecordId).Select(x => x.SubjectScore).Sum();            
                BLL.TestRecordService.UpdateTestRecordEndTime(this.TestRecordId, System.DateTime.Now, sumSubjectScore ?? 0);
